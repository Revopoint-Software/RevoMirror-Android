...
public class AppGridAdapter extends GenericGridAdapter<AppViewActivity.AppObject> {
    ...
    private ArrayList<AppViewActivity.AppObject> allApps = new ArrayList<>();
    ...
    public void updateHiddenApps(Set<Integer> newHiddenAppIds, boolean hideImmediately) {
        ...
        if (hideImmediately) {
            ...
            itemList.clear();
            for (AppViewActivity.AppObject app : allApps) {
    ...
    private static void sortList(List<AppViewActivity.AppObject> list) {
        Collections.sort(list, new Comparator<AppViewActivity.AppObject>() {
            @Override
            public int compare(AppViewActivity.AppObject lhs, AppViewActivity.AppObject rhs) {
                return lhs.app.getAppName().toLowerCase().compareTo(rhs.app.getAppName().toLowerCase());
            }
        });
    }

    public void addApp(AppViewActivity.AppObject app) {
        // Update hidden state
        app.isHidden = hiddenAppIds.contains(app.app.getAppId());
    ...
    public void removeApp(AppViewActivity.AppObject app) {
        itemList.remove(app);
        allApps.remove(app);
    }
    ...
    @Override
    public void populateView(View parentView, ImageView imgView, ProgressBar prgView, TextView txtView, ImageView overlayView, AppViewActivity.AppObject obj) {
