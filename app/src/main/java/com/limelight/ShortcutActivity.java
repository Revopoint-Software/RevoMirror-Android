...
public class ShortcutActivity extends BaseActivity {
        ...
        public void onServiceConnected(ComponentName className, IBinder binder) {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    if (computer == null) {
                        Dialog.displayDialog(ShortcutActivity.this,
                                        ...
                                        if (details.state == ComputerDetails.State.ONLINE && details.pairState == PairingManager.PairState.PAIRED) {
                                            ...
                                            if (app != null) {
                                                if (details.runningGameId == 0 || details.runningGameId == app.getAppId()) {
                                                    intentStack.add(ServerHelper.createStartIntent(ShortcutActivity.this, app, details, managerBinder));
                                                    ...
                                                    final Intent startIntent = ServerHelper.createStartIntent(ShortcutActivity.this, app, details, managerBinder);

                                                    UiHelper.displayQuitConfirmationDialog(ShortcutActivity.this, new Runnable() {
                                                        @Override
                                                        public void run() {
                                                ...
                                                Intent i;
                                                i = new Intent(ShortcutActivity.this, PcViewActivity.class);
                                                ...
                                                i.setClass(ShortcutActivity.this, AppViewActivity.class);
                                                intentStack.add(i);

                                                // If a game is running, we'll make the stream the top level activity
                                                if (details.runningGameId != 0) {
                                                    intentStack.add(ServerHelper.createStartIntent(ShortcutActivity.this,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        uuidString = getIntent().getStringExtra(AppViewActivity.UUID_EXTRA);
        String nameString = getIntent().getStringExtra(AppViewActivity.NAME_EXTRA);

        // App arguments, both are optional, but one must be provided in order to start an app
        String appIdString = getIntent().getStringExtra(GameActivity.EXTRA_APP_ID);
        String appNameString = getIntent().getStringExtra(GameActivity.EXTRA_APP_NAME);
        ...
        if (uuidString == null || uuidString.isEmpty()) {
            ...
            setIntent(new Intent(getIntent()).putExtra(AppViewActivity.UUID_EXTRA, uuidString));
        }

        if (appIdString != null && !appIdString.isEmpty()) {
            app = new NvApp(getIntent().getStringExtra(GameActivity.EXTRA_APP_NAME),
                    Integer.parseInt(appIdString),
                    getIntent().getBooleanExtra(GameActivity.EXTRA_APP_HDR, false));
        }
            ...
            try {
                ...
                setIntent(new Intent(getIntent()).putExtra(GameActivity.EXTRA_APP_ID, appId));
                app = new NvApp(
                        appNameString,
                        appId,
                        getIntent().getBooleanExtra(GameActivity.EXTRA_APP_HDR, false));
